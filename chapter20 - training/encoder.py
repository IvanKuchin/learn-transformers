...
class EncoderLayer(tf.keras.layers.Layer):
    def __init__(self, h, d_k, d_v, d_model, d_ff, rate, **kwargs):
        ...
        # below required only for build_graph
        self.d_model = d_model

    def build_graph(self):
        inputs = tf.keras.layers.Input(shape=(9, self.d_model))
        model = tf.keras.models.Model(inputs=inputs, outputs=self.call(inputs, None, True))
        return model
